...
async function fetchGeBIZ(query: string, limit = 15): Promise<GeBIZResponse> {
  ...
  try {
    ...
    return await res.json() as GeBIZResponse;
  } finally {
...
function formatRecords(
  ...
  records.forEach((r, i) => {
    ...
    if (supplier !== "N/A") lines.push(`    Supplier:   ${supplier}`);
    if (amount !== "N/A") lines.push(`    Amount:     ${amount}`);
    lines.push(`    Close Date: ${closeDate}`);
    if (awardDate !== "N/A") lines.push(`    Awarded:    ${awardDate}`);
    lines.push("");
  });
...
export async function gebizAdapter(options: ExtractOptions): Promise<AdapterResult> {
