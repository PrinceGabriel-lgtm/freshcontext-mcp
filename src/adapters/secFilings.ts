...
async function fetchSecFilings(query: string, maxResults = 10): Promise<SecSearchResponse> {
  ...
  try {
    ...
    return await res.json() as SecSearchResponse;
  } finally {
    clearTimeout(timeout);
  }
}

function formatFilings(data: SecSearchResponse, query: string, maxLength: number): AdapterResult {
  ...
  hits.forEach((hit, i) => {
    ...
    if (fileDate && fileDate !== "unknown") {
      if (!latestDate || fileDate > latestDate) latestDate = fileDate;
    }
